...
Это материал для подготовки к экзамену. По каждой теме:
**что это → как было у нас (с кодом) → почему именно так → подводные камни → проверьте себя**.
Список тем — в [Exam-Topics.md](Exam-Topics.md).

Экзамен проходит в формате разговора. От вас нужно объяснить **идею своими словами** и
показать, что понимаете, **зачем** оно нужно. Дословные определения учить не надо.
...
### Немного теории: как зависимость попадает в класс
Мы внедряли через **конструктор** — это основной способ. В новых проектах использовали
краткий синтаксис первичного конструктора:
```csharp
public class AccountService(IAccountRepo accountRepo, IPasswordHasher<AccountModel> hasher) : IAccountService
```
Здесь `accountRepo` и `hasher` — это и есть зависимости, которые контейнер найдёт в своих
регистрациях и подставит сам. Класс лишь объявляет «мне нужно вот это», но не знает, **какая
конкретно** реализация придёт и **где** она создаётся.

### Подводные камни
- **Забыли зарегистрировать сервис** в `Program.cs` → на запросе падает
  `Unable to resolve service for type '...'`. Очень частая ошибка: добавили новый сервис/
  репозиторий, но не дописали `AddScoped`. В ДЗ прямо напоминается: «не забываем добавить
  зависимости в `Program.cs`».
- **Захваченная зависимость (captive dependency).** Нельзя инжектить Scoped-сервис в
  Singleton: singleton переживёт запрос и «захватит» устаревший scoped-объект (например,
  мёртвый `DbContext`). Поэтому in-memory-сервис (Singleton) и sqlite-сервис (Scoped) имеют
  разный lifetime, а `DbContext` всегда Scoped.
- **Перепутали lifetime.** Сделать `SqliteNoteService` синглтоном — проект не соберётся
  (захватит scoped DbContext). Сделать `InMemoryNoteService` scoped — список заметок будет
  пересоздаваться на каждый запрос, и данные «не сохранятся».
...
### Немного теории: отложенное выполнение и отслеживание
- **Отложенное выполнение (deferred execution).** `IQueryable` сам по себе **не ходит в базу**
  — он только копит описание запроса. Реальный SQL уходит в момент `ToList()`/`ToArray()`/
  `FirstOrDefault()`/`foreach`. Поэтому в фильтре заметок мы спокойно собирали `query` из
  нескольких `Where`, и лишь в конце материализовали результат — база получила один готовый
  запрос.
- **Отслеживание изменений (change tracking).** По умолчанию `DbContext` запоминает
  загруженные сущности, и при `SaveChanges` сам видит, что поменялось. `AsNoTracking()`
  отключает это — для чтения быстрее и экономнее, но такую сущность **нельзя просто поменять и
  сохранить**, EF её не отслеживает.

### Немного теории: SQLite vs PostgreSQL — библиотека против сервера
Два разных по природе хранилища, и оба мы использовали:
- **SQLite** (Notes, PersonalAccount) — это **не отдельное приложение, а библиотека**, которая
  встраивается прямо в процесс приложения. Никакой отдельный «сервер БД» не запускается: вся
  база — это **один файл на диске** (`personal_account.db`), а обращается к нему наш код через
  встроенную библиотеку. Поэтому отдельно ставить и поднимать нечего — удобно для учёбы и
  небольших приложений.
- **PostgreSQL** (Marketplace) — это **отдельное приложение (сервер БД)**, работающее само по
  себе как самостоятельный процесс (у нас — в своём контейнере `database`). Приложение
  подключается к нему **по сети**, по адресу и порту (`Host=database;Port=5432;...`). Такой
  сервер обслуживает сразу несколько клиентов, держит блокировки и транзакции (`FOR UPDATE` из
  темы 11) — поэтому покупки в Marketplace на Postgres, а не на SQLite.

Главная мысль: **SQLite живёт внутри самого приложения (файл + встроенная библиотека), а
...
- **Забыли `SaveChanges`** → «удалил/добавил/изменил, а в базе всё по-старому». Самая частая
  ошибка с EF.
- **Изменили сущность, полученную с `AsNoTracking`, и ждём, что `SaveChanges` сохранит** — не
  сохранит. No-tracking только для чтения. (В нашем `Repo.UpdateAsync` поэтому сущность
  заново берётся через `FindAsync`, который отслеживается.)
- **Забыли миграцию после правки модели** → код ждёт новую колонку, а в базе её нет → ошибка
  на запросе. Поменял модель → `migrations add` → `database update`.
- **`enum` в EF Core по умолчанию хранится числом** (`Admin → 0`, `Student → 1`). Всплыло в
  ДЗ №5 при ручной вставке студентов в SQLite: в SQL надо писать число, а не «Admin».
- **Фильтрация в памяти вместо запроса.** Если сделать `_context.Notes.ToList().Where(...)` —
  EF сначала вытащит **всю** таблицу, и только потом отфильтрует в памяти. Разница `List.Where`
  (в памяти) vs `IQueryable.Where` (уезжает в SQL) одинаково выглядит, но по нагрузке —
...
### Немного теории: идемпотентность
GET, PUT, DELETE считаются **идемпотентными** — повторить их можно без вреда (несколько раз
запросить страницу или удалить один и тот же id — результат тот же). POST — **нет**: повторная
отправка создаёт/делает ещё раз. Именно поэтому браузер переспрашивает «отправить форму
повторно?» при F5 на ответе POST. PRG убирает эту проблему, переводя пользователя на
безопасный GET.

### Подводные камни
- **Меняем данные через GET** (например, удаление по ссылке `/delete?id=5`) — так делать
  нельзя: предзагрузка ссылок браузером, поисковые роботы или просто обновление страницы
  случайно «нажмут» это действие. Изменение → только POST.
- **Несовпадение `name` и параметра.** Если в форме `name="noteId"`, а в обработчике параметр
  `int id` — связывание не сработает, придёт `0`/`null`. Имена должны совпадать.
...
- **При невалидной форме сделать `RedirectToAction`, а не `return View(model)`** — тогда
  пропадут и сообщения об ошибках, и уже введённые пользователем данные (после редиректа это
  новый запрос с пустым ModelState). Правильно — вернуть ту же View с той же моделью.
- **`[Required]` на `string` ловит `null`/пусто, но не пробелы.** «   » формально не пустая
  строка — если это важно, нужна доп. проверка/`[StringLength]`.
...
### Немного теории: почему хэш, а не шифрование
Шифрование **обратимо** (есть ключ — расшифровал), хэш — **нет**. Для паролей нужна именно
необратимость: даже мы сами не должны уметь восстановить пароль из базы. Хорошие хэшеры
паролей (как `PasswordHasher`) ещё и добавляют **соль** (salt) — случайную добавку к каждому
паролю, поэтому одинаковые пароли двух пользователей дают **разные** хэши, и заранее
посчитанные таблицы хэшей не помогают.

### Подводные камни
- Путают аутентификацию и авторизацию — это разные шаги (кто ты / что можно).
- **Забыли `app.UseAuthentication()`** (или поставили после `UseAuthorization`) → `User`
  всегда анонимный, `[Authorize]` валит всех, claims не читаются. Классическая ошибка с
  пайплайном.
- **Роль-строка не совпадает.** В claim кладём `account.Role.ToString()` (т.е. `"Admin"`), и в
  `[Authorize(Roles = "Admin")]` строка должна совпадать буква-в-букву. Опечатка → доступ не
  работает.
...
- (посложнее) «Почему `UseAuthentication` идёт раньше `UseAuthorization`?»
...
- **Gateway = единственная публичная дверь.** Наружу торчит только он (`ports: 6139:80`), а сами
  сервисы открыты лишь внутри сети compose (`expose: 8080`) — снаружи до них напрямую не
  достучаться. Это безопаснее и позволяет менять/добавлять сервисы за gateway, не трогая клиент.
...
- **Контейнер самодостаточен.** Внутри образа лежит и приложение, и нужный рантайм (.NET) — на
  сервере не надо вручную ставить ни .NET, ни саму базу. Контейнеры изолированы друг от друга и
  общаются по внутренней сети compose по именам (`database`, `products-api`).
- **База — отдельный сервис.** PostgreSQL крутится в своём контейнере `database` как
  самостоятельный сервер, а остальные сервисы ходят в него по сети (`Host=database;Port=5432`).
  В этом и разница с SQLite, который был просто встроенной библиотекой (подробнее — в теме 4).
...
- (посложнее) «Три сервиса проверяют одну cookie — как договорились?» → общий ключ
  DataProtection.
...
- Блокировать нужно именно те строки, которые меняем (товар, покупатель, продавец) — иначе
  гонка остаётся.
- **Проверка и изменение порознь, без блокировки** — это и есть источник гонки: прочитали
  `Amount = 1`, между чтением и записью кто-то уже купил. Блокировка (`FOR UPDATE`) или
  транзакция нужны, чтобы «прочитал → проверил → записал» было неделимым.
- **Слишком долгая транзакция держит блокировки** дольше нужного и тормозит остальных —
  внутри транзакции делаем только то, что обязано быть атомарным, без лишней работы.

### Проверьте себя
- «Почему покупку оборачиваем в транзакцию?» → атомарность.
- «Что будет без неё при ошибке на середине?» → неконсистентные данные.
- (посложнее) «От чего спасает `FOR UPDATE` при двух одновременных покупках?» → race
...
### Подводные камни
- Глубоко копать не нужно — достаточно понимать «компонент + props + поход в API за JSON».
...
- **Pull Request** — способ показать и обсудить изменения (виден diff) перед вливанием. Ровно
  ...
  изменения через PR.
- **Коммиты — это история и точки возврата.** Каждый коммит — снимок проекта: видно, что и
  когда менялось, и можно откатиться к рабочему состоянию. Поэтому коммитим осмысленными
  порциями («создал сервис — коммит», «добавил миграцию — коммит»), а не всё одной кучей в
  конце — так историю проще читать и откатывать точечно.
...
- **Закоммитили лишнее** (`bin/`, `obj/`, `.idea/`, секреты) — для этого есть `.gitignore`:
  генерируемые файлы и личные настройки IDE в репозиторий не кладём.
...
- «Зачем вообще коммиты и почему лучше делать их маленькими и осмысленными?» → история и откат.

---

## Частые ошибки — чего избегать

Короткий список самых частых ошибок по всем темам. Пробегитесь по нему перед экзаменом и
проверьте, не наступаете ли вы на эти грабли в своём коде.
...
| DI | Перепутали Singleton/Scoped | In-memory теряет данные / sqlite не собирается (captive dependency) |
...
| Auth | Нет/не там `UseAuthentication` | `User` анонимный, `[Authorize]` валит всех |
...
| Секреты | Положили пароль в `appsettings.json` | Утечка через Git; секреты — в user-secrets/env |
